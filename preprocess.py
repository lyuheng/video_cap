...
def readfeat(train=True, data_dir=None):
    ...
    for d in data:
        ...
        if train:
            sys.stdout.write("\rprocessing features {0:.2f}%...  ".format((i*100)/1450 )) # 1450 in total
        else:
            sys.stdout.write("\rprocessing features {0:.2f}%...  ".format(i))             # 100 in total
    print('')
...
def label2onehot(label, limit=100):        # 把每句话变成onehot
    v = load_obj('vocabindex')      # dic
    onehot = []
    i = 1
    for video in label:
        onehot.append([])
...
def vocab():
    ...
    for labels in l:
        for label in labels['caption']:
            ...
            #print(words)
    print(v)
    save_obj(v, 'vocabcount')        # training_label每个词出现的次数
        #labels.append(label['caption'])
...
def label(threshold=0):
    ...
    v1['<unk>'] = i+2
    v2[i+2] = '<unk>'
    save_obj(v1, 'vocabindex')          # dic
    save_obj(v2, 'index2vocab')         # reverse dic
    print(v1)
    print(len(v1))
